...
function CreateUserAdministrator(props:any) {
    ...
    const handleSubmit = (event:any) => {
        event.preventDefault();
        props.createUser({
                userType, subType, password, ssn, fName, lName, phone,
            gender, honorific, birthdate, houseNum, street, city, state, country, zip
        });
    }

    function disableSubmit() {
        return !(password && subType && ssn && fName && lName && phone &&
            gender && honorific && birthdate && houseNum && street && city && state && country && zip)
    }
        ...
        return <Fragment>
            <div>
                <div style={{display:"inline-block", textAlign:'left'}}>
                    ...
                    <div className={'div-table-row'} style={{backgroundColor:"transparent", display:"flex", width:"100%"}}>
                        <label className={'div-table-col'} style={{fontWeight:"bold", color:"black"}}>Priority Level: </label>
                        <select className={'div-table-col'} style={{marginLeft:"auto", marginRight:0}}
                                value={subType}
                                onChange={handleChangeSubType}>
                            <option value="">Select option</option>
                            <option value={'1'}>Primary Administrator</option>
                            <option value={'0'}>Secondary Administrator</option>
                        </select>
                    </div>
                    ...
                    <div className={'div-table-row'} style={{backgroundColor:"transparent",display:"flex", width:"100%"}}>
                        ...
                        <input className={'div-table-col'} style={{marginLeft:"auto", marginRight:0}}
                               pattern={'[0-9]{5}'} type={'number'} value={zip} onChange={handleChangeZip}/>
                    </div>
                    <div className={'div-table-row'} style={{backgroundColor:"transparent",display:"flex", width:"100%"}}>
